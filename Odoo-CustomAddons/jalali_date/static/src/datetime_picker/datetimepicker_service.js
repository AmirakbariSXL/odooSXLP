  ...
  start(env, { popover: popoverService }) {
    return {
      ...
      create: (
        ...
        const apply = () => {
          if (
            areDatesEqual(lastInitialProps?.value, deepCopy(pickerProps.value))
          ) {
            return;
          }
            ...
            (el, currentValue) => {
              ...
              if (el.value.split(" ")[1]) {
                if (el.value.split(" ")[0].split("/")[2]) {
                  jalali_value = el.value.split(" ")[0].split("/");

                  const gresult = farvardin.solarToGregorian(
                    parseInt(jalali_value[0]),
                    parseInt(jalali_value[1]),
                    parseInt(jalali_value[2]),
                  );
                  gregorian_value = `${gresult[0]}/${gresult[1]}/${gresult[2]} ${el.value.split(" ")[1]}`;
                  ...
                  gregorian_value = `${gresult[0]}-${leftPad(gresult[1], 2)}-${leftPad(gresult[2], 2)} ${el.value.split(" ")[1]}`;
                }
              } else {
                if (el.value.split("/")[2]) {
                  jalali_value = el.value.split("/");
                  const gresult = farvardin.solarToGregorian(
                    parseInt(jalali_value[0]),
                    parseInt(jalali_value[1]),
                    parseInt(jalali_value[2]),
                  );
                  gregorian_value = `${gresult[0]}/${gresult[1]}/${gresult[2]}`;
                  ...
                  gregorian_value = `${gresult[0]}-${leftPad(gresult[1], 2)}-${leftPad(gresult[2], 2)}`;
                }
              }
